...
**Why Vi-Qwen2-7B-RAG over alternatives:**

| Candidate | Reason Rejected/Selected |
|-----------|-------------------------|
| **AITeamVN/Vi-Qwen2-7B-RAG** ✅ | Purpose-built for Vietnamese RAG tasks. Fine-tuned on Vietnamese QA pairs with retrieval-augmented context. Fits in single 8GB GPU with NF4 quantization. |
| Vistral-7B-Chat | General Vietnamese chat model, not optimized for RAG/extraction tasks. Lower extractive QA performance in our pilot tests. |
| Sailor-7B | Multilingual SEA model — Vietnamese is one of many languages, less specialized. Larger context but worse Vietnamese factuality. |
| GPT-4 / Gemini API | Higher quality but introduces latency, cost, and external dependency. Unsuitable for reproducible offline evaluation. Rate limits conflict with multi-trajectory voting. |
| Smaller models (1-3B) | Insufficient reasoning capability for multi-hop questions requiring 2-3 inference steps. |

**Why GreenNode-Embedding-Large-VN-Mixed-V1:** Only production-quality Vietnamese-specific embedding model at 1024 dimensions. Benchmarked highest on Vietnamese STS tasks among open models (vs. multilingual-e5, BGE-m3).

### Data Privacy and Legal Compliance

- **Source data license**: Vietnamese Wikipedia content is released under CC-BY-SA 3.0. Our system preserves attribution by storing `page_title` and `page_url` with every chunk, satisfying the attribution requirement.
- **No personal data processing**: The system ingests publicly available encyclopedia articles. No user-generated PII is collected or stored.
- **Generated QA datasets**: Derived from CC-BY-SA content; output datasets inherit the same license.
- **Model weights**: All models used (Vi-Qwen2-7B-RAG, GreenNode, bge-reranker-v2-m3) are released under permissive licenses (Apache 2.0 or MIT).

## 4.5 Evaluation Metrics

### Target Justification

Metric targets are derived from baseline performance and comparable systems:

- **Hit Rate > 70%**: Our best baseline (BM25+Vector, no graph) achieves 66%. A +4–6% improvement demonstrates statistically meaningful value of graph signal. Published GraphRAG systems (Microsoft GraphRAG 2024, GRAFT) report 5–15% gains over dense-only retrieval.
- **MRR > 0.50**: Baseline MRR is ~0.48. Target represents a modest but verifiable improvement, consistent with BEIR benchmark expectations for domain-specific retrieval.
- **EM > 30%, F1 > 50%**: Aligned with UIT-ViQuAD 2.0 leaderboard results for extractive QA on Vietnamese text (top systems achieve ~35% EM, ~65% F1 with XLM-R).
