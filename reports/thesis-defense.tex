...
\date{June 2026}
\titlegraphic{\includegraphics[height=1.2cm]{../docs/logo/fpt_logo.png}}
...
\begin{itemize}
  ...
  \item \textbf{Hallucination} --- 57\% unfaithful citations (Liu et al., 2024)
...
\begin{itemize}
  \item Typed Neo4j KG + 3 pluggable NER backends
  \item Hybrid retrieval + cross-encoder reranking
  \item ReAct agent with 4 deterministic tools
\end{itemize}
\end{exampleblock}
\vspace{0.4em}
...
\begin{tikzpicture}[
  ...
  \node[box, fill=emphasis!10, below=1.2cm of react] (vec) {BM25 + Dense};
...
\textbf{3 Pluggable NER Backends}
\begin{itemize}
  \item \textbf{Simple} --- regex heuristic + keyword type classification
  \item \textbf{Underthesea} --- BIO tagging, offline Vietnamese NER
  \item \textbf{PhoNLP} --- VnCoreNLP word segmentation + PhoNLP NER
\end{itemize}
\vspace{0.3em}
...
Token F1 $>$ 0.40 \quad | \quad Context Hit Rate $>$ 0.60 \quad | \quad Abstain Accuracy $>$ 0.60
\end{block}
\end{frame}
...
\textbf{Baselines}
\begin{itemize}
  \item[\con{B1}] Vector-only (dense retrieval)
  \item[\con{B2}] BM25-only (sparse retrieval)
  \item[\con{B3}] Graph-only (Cypher traversal)
  \item[\pos{Ours}] Hybrid (graph + text + reranking)
\end{itemize}
\end{columns}
\vspace{0.5em}
\begin{alertblock}{Results (Placeholder)}
\centering \textit{Awaiting full evaluation run --- targets: F1 $>$ 0.40, Hit Rate $>$ 0.60, Latency $<$ 3s}
\end{alertblock}
\end{frame}
...
\begin{itemize}
  \item End-to-end local GraphRAG
  \item Typed Vietnamese KG (998p, 19K chunks)
  \item ReAct agent + citation tracking
  \item Dataset pipeline + ViQuAD2
  \item Multi-metric evaluation framework
\end{itemize}
\end{exampleblock}
...
\begin{itemize}
  \item QLoRA fine-tuning (Text2Cypher)
  \item DPO alignment (tool compliance)
  \item Community detection (Louvain)
  \item Query decomposition (StepChain)
  \item Scale to 5000+ pages
\end{itemize}
\end{alertblock}
\end{columns}
\end{frame}
