...
def test_speech_transcribe_endpoint_success(monkeypatch) -> None:
    monkeypatch.setattr(main.settings, "app_api_key", None)

    def _fake_transcribe(audio_bytes: bytes, *, content_type: str | None = None):
    ...
    monkeypatch.setattr(main, "transcribe_audio_bytes", _fake_transcribe)
...
def test_speech_transcribe_endpoint_runtime_error(monkeypatch) -> None:
    monkeypatch.setattr(main.settings, "app_api_key", None)

    def _fail(_audio_bytes: bytes, *, content_type: str | None = None):
        raise RuntimeError("Speech-to-text is unavailable")

    monkeypatch.setattr(main, "transcribe_audio_bytes", _fail)
    ...
    assert "Speech-to-text is unavailable" in resp.json()["detail"]


def test_speech_transcribe_endpoint_model_init_failure(monkeypatch) -> None:
    monkeypatch.setattr(main.settings, "app_api_key", None)

    def _fail(_audio_bytes: bytes, *, content_type: str | None = None):
        raise RuntimeError("Speech-to-text model initialization failed: offline")
    ...
    assert resp.status_code == 400
    assert "model initialization failed" in resp.json()["detail"]
